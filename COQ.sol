...
// File: contracts/COQDividendPayingToken.sol
pragma solidity ^0.7.6;
...
contract COQ is ERC20, Ownable {
    ...
    COQDividendTracker public dividendTracker;
    ...
    // timestamp for when the token can be traded freely on PCCS
    ...
    // addresses that can make transfers before presale is over, if any
    ...
    constructor() public ERC20("COQ", "COQ") {
        // LP address
        LPaddress = address(0x927b12207643A4613b3E4E2534FB991C46d9cb3D);
        uint256 _processDividendTime = block.timestamp;
        processDividendTime = _processDividendTime;

        dividendTracker = new COQDividendTracker();
        ...
        */
        _mint(owner(), 1500000000000 * (10**decimals()));
    }
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "COQ: The dividend tracker already has that address");

        COQDividendTracker newDividendTracker = COQDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "COQ: The new dividend tracker must be owned by the COQ token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "COQ: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "COQ: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "COQ: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "COQ: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "COQ: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "COQ: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "COQ: This account cannot send tokens until trading is enabled");
        }
...
contract COQDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("COQ_Dividend_Tracker", "COQ_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "COQ_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "COQ_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main COQ contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "COQ_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "COQ_Dividend_Tracker: Cannot update claimWait to same value");
